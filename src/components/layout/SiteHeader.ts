const initHeader = () => {
  ...
  const triggers = [
    ...header.querySelectorAll<HTMLButtonElement>("[data-mega-trigger]"),
  ];
  ...
  const setInactive = (element: HTMLElement) => {
    element.hidden = true;
  ...
  const closeDesktop = (restore = false) => {
    const trigger = triggers.find(
      (item) => item.dataset.megaTrigger === active
    );
    active = null;
  ...
  const openDesktop = (id: string, focusFirst = false) => {
    ...
    triggers.forEach((item) =>
      item.setAttribute(
        "aria-expanded",
        String(item.dataset.megaTrigger === id)
      )
    );
    panels.forEach((panel) =>
      panel.dataset.megaPanel === id ? setActive(panel) : setInactive(panel)
    );
    menu.hidden = false;
    menu.setAttribute("aria-hidden", "false");
    if (focusFirst)
      panels
        .find((panel) => panel.dataset.megaPanel === id)
        ?.querySelector("a")
        ?.focus();
  };
  triggers.forEach((trigger) => {
    trigger.addEventListener("click", () =>
      active === trigger.dataset.megaTrigger
        ? closeDesktop()
        : openDesktop(trigger.dataset.megaTrigger!)
    );
    trigger.addEventListener("keydown", (event) => {
      if (event.key === "ArrowDown") {
        event.preventDefault();
        openDesktop(trigger.dataset.megaTrigger!, true);
      }
  ...
  panels.forEach((panel) =>
    panel.addEventListener("keydown", (event) => {
      ...
      const trigger = triggers.find(
        (item) => item.dataset.megaTrigger === panel.dataset.megaPanel
      );
      if (event.key === "Escape") {
        event.preventDefault();
        closeDesktop(true);
  ...
  document.addEventListener("pointerdown", (event) => {
    if (active && !header.contains(event.target as Node)) closeDesktop();
  });
  header.addEventListener("focusout", () =>
    requestAnimationFrame(() => {
      if (active && !header.contains(document.activeElement)) closeDesktop();
    })
  );
  ...
  const mobileTriggers = [
    ...mobile.querySelectorAll<HTMLButtonElement>("[data-mobile-trigger]"),
  ];
  const closeMobileSections = (except?: string) =>
  ...
  mobile.addEventListener("toggle", () => {
    if (!mobile.open) closeMobileSections();
  });
  mobile.addEventListener("keydown", (event) => {
    if (event.key !== "Escape") return;
    const expanded = mobileTriggers.find(
