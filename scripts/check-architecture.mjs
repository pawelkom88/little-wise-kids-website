...
const siteImports = [...read("src/styles/site.css").matchAll(/@import\s+"([^"]+)";/g)]
  .map((match) => match[1]);
...
const ownedStyles = [
  ...
  ["src/components/sections/Values/ValuesDifference.astro", "./ValuesDifference.css"],
...
for (const path of astroFiles) {
  ...
  assert(!/\bis:inline\b/.test(source), `${path} bypasses Astro script processing`);
...
const typographyDeclaration = /^\s*(?:font-size|font-weight|line-height|letter-spacing):\s*([^;]+);/gim;
...
assert(
  read("src/components/layout/SiteHeader.astro").includes('<script src="./SiteHeader.ts"></script>'),
  "SiteHeader must load its processed TypeScript module",
);
assert(
  read("src/components/sections/GallerySection.astro").includes('<script src="./GallerySection.ts"></script>'),
  "GallerySection must load its processed TypeScript module",
);

const headerMarkup = read("src/components/layout/SiteHeader.astro");
assert(
  headerMarkup.includes("data-mobile-close") && headerMarkup.includes('aria-label="Close menu"'),
...
assert(
  galleryMarkup.includes('role="group"') && galleryMarkup.includes('aria-labelledby="gallery-title"'),
  "Gallery carousel must have a role and accessible name",
);

const sourceFiles = walk("src").filter((path) => /\.(?:astro|css|ts)$/.test(path));
const sourceText = sourceFiles.map(read).join("\n");
const globalTokens = [...read("src/styles/tokens.css").matchAll(/^\s*(--[a-z0-9-]+):/gm)]
  .map((match) => match[1]);
const unusedTokens = globalTokens.filter((token) => !sourceText.includes(`var(${token})`));

assert.deepEqual(unusedTokens, [], `Unused global tokens: ${unusedTokens.join(", ")}`);

for (const path of sourceFiles) {
  for (const match of read(path).matchAll(/["'](\/assets\/[^"']+)["']/g)) {
    const assetPath = join("public", match[1]);
    assert(existsSync(join(root, assetPath)), `${relative(root, path)} references missing ${match[1]}`);
  }
}
